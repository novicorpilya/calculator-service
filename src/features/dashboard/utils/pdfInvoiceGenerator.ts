...
export const generateInvoicePDF = (entity: CalculationEntity) => {
    ...
    if (adjustments.global_margin && adjustments.global_margin !== 1) {
        doc.text(`Наценка/Скидка: ${(adjustments.global_margin * 100 - 100).toFixed(1)}%`, pageWidth - margin, currentY, { align: 'right' });
        currentY += 7;
    }
    if (adjustments.delivery_cost) {
        doc.text(`Доставка: ${adjustments.delivery_cost.toLocaleString()} RUB`, pageWidth - margin, currentY, { align: 'right' });
        currentY += 7;
    }
    if (adjustments.service_cost) {
        doc.text(`Доп. услуги: ${adjustments.service_cost.toLocaleString()} RUB`, pageWidth - margin, currentY, { align: 'right' });
        currentY += 7;
    }
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text(`ИТОГО: ${entity.totalCost.toLocaleString()} RUB`, pageWidth - margin, currentY + 5, { align: 'right' });
