...
export const ProductPickerModal: React.FC<ProductPickerModalProps> = ({
    ...
    const filteredCatalog = useMemo(() => {
        if (!Array.isArray(catalog)) {
            console.warn('[ProductPickerModal] catalog is not an array:', catalog);
            return [];
        }
        ...
        <div className="fixed inset-0 bg-background/80 backdrop-blur-xl z-[150] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
            <div className="glass-card max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-3xl border-primary/20">
                ...
                <div className="p-8 border-b border-border-theme flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-[10px] font-black text-foreground/40 uppercase tracking-widest">
                            {currentItem ? (
                                <>
                                    Замена для:{' '}
                                    <span className="text-primary">{currentItem.inventory}</span>
                                </>
                            ) : (
                    ...
                    <button
                        onClick={onClose}
                        className="p-3 bg-card border border-border-theme rounded-xl hover:text-primary transition-all"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar">
                    {filteredCatalog.length > 0 ? (
                        filteredCatalog.map((item) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`group relative p-6 rounded-[2rem] border-2 transition-all cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-6 ${
                                        isCurrent
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-primary bg-primary/10 group-hover:scale-110 transition-transform duration-500 shrink-0">
                                            <Package size={24} />
                                            <div
                                                className="absolute bottom-4 left-6 w-8 h-1"
                                                style={{ backgroundColor: item.color }}
                                            />
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[15px] font-black tracking-tight leading-none group-hover:text-primary transition-colors">
                                                {item.name}
                                            </p>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="text-right">
                                            ...
                                            <p
                                                className={`text-sm font-black ${item.stock < 10 ? 'text-red-500' : 'text-foreground'}`}
                                            >
                                                {item.stock} шт
                                            </p>
                                        </div>
                        ...
                        <div className="py-20 text-center space-y-4">
                            ...
                            <p className="text-[11px] font-black uppercase tracking-widest text-foreground/40 italic">
