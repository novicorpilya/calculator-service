...
import { type Calculation, type InventoryItem } from '../../dashboard.types';
 ...
 * Handles procurement summaries, catalog browsing, and filtering by venue.
 */
export const InventoryManager = React.memo<InventoryManagerProps>(({ calculations, venues }) => {
    ...
    const procurementData = useMemo(() => {
        const filteredCalcs = calculations.filter(c => {
            ...
            // Trying to match by name if venue mapping isn't formal yet
        ...
        const aggregated = filteredCalcs
            ...
            .reduce((acc, item) => {
                const key = `${item.inventory}-${item.color}`;
                if (!acc[key]) {
            ...
            {activeTab === 'procurement' ? (
                                        ...
                                        <tr key={i} className="border-b border-border-theme hover:bg-primary/5 transition-colors group">
                                            <td className="px-8 py-6">
                                                <div className="space-y-1">
                                                    ...
                                                    <p className="text-[9px] font-bold text-foreground/30 uppercase tracking-widest">Цена за ед: {item.price.toLocaleString()} ₽</p>
                                                </div>
                                            </td>
                                            <td className="px-8 py-6">
                                            ...
                                            <td colSpan={4} className="px-8 py-20 text-center">
                                                <div className="space-y-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-[0.2em] mt-1">Убедитесь, что у вас есть утвержденные (Approved) расчеты</p>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {isLoadingCatalog ? (
                        ...
                        <div key={item.id} className="glass-card p-8 space-y-8 group hover:border-primary/20 transition-all">
                            <div className="flex items-start justify-between">
                                <div className="p-4 rounded-2xl bg-primary/5 text-primary">
                                    <Package size={24} />
                                </div>
                                <div className="flex items-center gap-2 bg-foreground text-background px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center gap-3 bg-card border border-border-theme p-3 rounded-2xl w-fit">
                                    <div className="w-4 h-4 rounded-full shadow-inner" style={{ backgroundColor: item.color }} />
                                    <span className="text-[10px] font-black uppercase tracking-widest text-foreground/60">Маркировка: {item.color}</span>
                                </div>
                            </div>

                            <div className="space-y-4 pt-6 border-t border-border-theme">
                                <p className="text-[9px] font-black text-foreground/40 uppercase tracking-[0.2em]">Нормативы (на 100м²/100 чел):</p>
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black">{item.norm_area}</p>
                                        <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-widest truncate">Площадь</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black">{item.norm_personnel}</p>
                                        <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-widest truncate">Персонал</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black">{item.norm_intensity}</p>
                                        <p className="text-[8px] font-bold text-foreground/20 uppercase tracking-widest truncate">Трафик</p>
                                    </div>
                                </div>
                            </div>

                            <button className="w-full py-4 rounded-2xl bg-card border border-border-theme text-[10px] font-black uppercase tracking-widest hover:border-primary hover:text-primary transition-all flex items-center justify-center gap-2">
                                <Info size={14} /> Тех. характеристики
                            </button>
                        </div>
                    ))}
                </div>
