...
export const CalculationBreakdown = React.memo(
    ({ item, hidePrices, displayMode }: CalculationBreakdownProps) => {
        ...
        const mode = displayMode || (hidePrices ? 'hidden' : 'exact');
        const showPrices = mode !== 'hidden';
        ...
        return (
            <div className="group relative glass-card !bg-white/[0.01] border-white/5 hover:border-primary/20 transition-all duration-300 shadow-xl overflow-hidden">
                {/* Visual Accent Line */}
                <div
                    className="absolute top-0 left-0 w-1 lg:w-1.5 h-full opacity-60"
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        ...
                        {showPrices && (
                            <div className="flex flex-col sm:items-end">
                                <p className="text-[9px] font-black text-foreground/50 uppercase tracking-[0.2em] mb-1">
                                    Цена за единицу
                                </p>
                                <span className="text-lg font-black text-foreground/80">
                                    {mode === 'range' ? (
                                        <>
                                            {Math.round(item.price * 0.85).toLocaleString()}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 space-y-3">
                            ...
                            <p className="text-2xl font-black text-foreground/90 leading-none">
                                ...
                                <span className="text-xs font-bold text-foreground/50">ШТ</span>
                            </p>
                        </div>
                        ...
                        <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4 space-y-3">
                            ...
                            <p className="text-2xl font-black text-primary leading-none">
                                ...
                                <span className="text-xs font-bold opacity-30">ШТ</span>
                            </p>
                        </div>
                        ...
                        {showPrices && (
                            <div className="sm:col-span-2 bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col justify-between">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <span className="text-[10px] font-bold opacity-20 uppercase">
                                        Прогноз
                                    </span>
                                </div>
                                <div className="flex items-end justify-between">
                                    <p className="text-2xl font-black text-foreground/90 leading-none">
                                        {mode === 'range' ? (
                                            <>
                                                {Math.round(annualBudget * 0.85).toLocaleString()}
                                                <span className="text-foreground/40 mx-1">-</span>
                                                {Math.round(annualBudget * 1.15).toLocaleString()}
                                                <span className="text-xs font-bold opacity-30 ml-1">
                                                    ₽
                                                </span>
                                            </>
                                        ) : (
                                    ...
                                    <p className="text-[10px] font-bold text-foreground/50 italic">
                                        ~
                                        {annualConsumption < 1
                                            ? annualConsumption.toFixed(1)
                                            : Math.ceil(annualConsumption)}{' '}
                                        ед.
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="flex items-center gap-2 px-3 py-1.5">
                                ...
                                <span className="text-[11px] font-black text-foreground/60">
                                    {item.norms?.replacementCycle || 30} дн.
                                </span>
                            </div>
                        </div>

                        <div className="hidden sm:block">
                            <span className="text-[9px] font-bold text-foreground/10 uppercase tracking-[0.4em]">
                                СТАНДАРТ ISO 18406
                            </span>
                        </div>
                    </div>
