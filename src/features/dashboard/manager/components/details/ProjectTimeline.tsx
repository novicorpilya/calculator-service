...
export const ProjectTimeline: React.FC<ProjectTimelineProps> = ({ calculationId }) => {
    ...
    const getIcon = (action: string) => {
        switch (action) {
            case 'invitation_created': return <User size={14} />;
            case 'calculation_status_force_updated':
            case 'project_status_update': return <ArrowRightLeft size={14} />;
            case 'margin_update': return <TrendingUp size={14} />;
            case 'calculation_manager_assigned': return <CheckCircle2 size={14} />;
                ...
                <div key={event.id} className="relative flex gap-6 group">
                    ...
                    <div className="flex-1 space-y-1.5 pb-2">
                        <div className="flex items-center justify-between">
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/30 flex items-center gap-1.5">
                            ...
                            <div className="p-3 bg-foreground/[0.02] rounded-xl border border-border-theme/40 text-[11px] text-foreground/50 font-medium">
                                {Object.entries(event.details).map(([key, val]) => (
                                    <div key={key} className="flex gap-2">
                                        <span className="font-black uppercase text-[9px] tracking-widest opacity-40">{key}:</span>
