...
import { ArrowRight, Activity, ArrowUpRight, Users, Clock, Zap, MessageSquare } from 'lucide-react';
import { RevenuePulseChart, PortfolioRadarChart, StatusEfficiencyChart } from './analytics';
...
export const ManagerOverview = React.memo<ManagerOverviewProps>(
    ({ calculations, onNavigate, onSelect }) => {
        const [timeRange, setTimeRange] = React.useState<'week' | 'month' | 'quarter' | 'all'>(
            'all'
        );
        ...
        const stats = useMemo(() => {
            ...
            const pending = filteredData.filter(
                (c) => c.status === 'sent' || c.status === 'revision' || c.status === 'expert'
            );
            ...
            return {
                ...
                conversion:
                    filteredData.length > 0
                        ? Math.round(
                              ((invoiced.length + implementation.length + completed.length) /
                                  filteredData.length) *
                                  100
                          )
                        : 0,
                    ...
                    filteredData.length > 0
                        ? Math.round(
                              filteredData.reduce((sum, c) => {
                                  const cost =
                                      c.totalCost || calculateTotalCost(c.results?.summary || []);
                                  return sum + cost;
        ...
        const chartData = useMemo(() => {
            ...
            if (timeRange === 'week') {
                ...
                periodicData = Array.from({ length: 7 }).map((_, i) => {
                    ...
                    const volume = projects.reduce(
                        (sum, c) =>
                            sum + (c.totalCost || calculateTotalCost(c.results?.summary || [])),
                        0
                    );
                ...
                periodicData = rollingMonths.map((dateObj) => {
                    ...
                    const volume = projectsInMonth.reduce(
                        (sum, c) =>
                            sum + (c.totalCost || calculateTotalCost(c.results?.summary || [])),
                        0
                    );
            ...
            const typeData = OBJECT_TYPES.map((type) => {
                ...
                return {
                    ...
                    fullMark: Math.max(
                        ...OBJECT_TYPES.map(
                            (t) => filteredData.filter((c) => c.type === t.value).length
                        ),
                        5
                    ),
                };
            ...
            const statusMap: Record<string, { label: string; order: number; keys: string[] }> = {
                ...
                paid: {
                    label: 'Оплата',
                    order: 6,
                    keys: ['paid', 'payment_review', 'payment_rejected'],
                },
                shipping: {
                    label: 'Логистика',
                    order: 7,
                    keys: ['processing', 'sent_to_warehouse', 'ready', 'shipping'],
                },
            ...
            <div className="space-y-8 sm:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 sm:gap-6">
                    <div className="space-y-1 sm:space-y-2">
                        <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.3em] sm:tracking-[0.4em] text-primary">
                            Live Dashboard
                        </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 bg-card border border-border-theme p-2 sm:p-3 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl backdrop-blur-md w-full sm:w-auto">
                        <div className="overflow-x-auto scrollbar-hide">
                            <div className="flex items-center gap-1 bg-foreground/5 p-1 rounded-[1.2rem] sm:rounded-[1.5rem] min-w-max">
                                {[
                                    { id: 'week', label: 'Нед' },
                                    { id: 'month', label: 'Мес' },
                                    { id: 'quarter', label: 'Кв' },
                                    ...
                                    <button
                                        ...
                                        className={`px-3 sm:px-4 py-2 rounded-lg sm:rounded-[1.2rem] text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${
                                            timeRange === range.id
                            ...
                            {recentItems.length > 0 ? (
                                recentItems.map((calc, idx) => (
                                    <div
                                        ...
                                        onClick={() =>
                                            onSelect ? onSelect(calc.id) : onNavigate('pipeline')
                                        }
                                    >
                                        <div className="flex flex-col items-center gap-3">
                                            {(() => {
                                                const isUrgent = [
                                                    ...
                                                    'payment_review',
                                                    'expert',
                                                ].includes(calc.status);
                                                return (
                                                    <div
                                                        className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-sm transition-all group-hover/item:shadow-xl ${isUrgent ? 'bg-orange-500 dark:bg-orange-500/20 text-white dark:text-orange-500 border-orange-500/10' : 'bg-primary dark:bg-primary/20 text-white dark:text-primary border-primary/10'}`}
                                                    >
                                                        {isUrgent ? (
                                                            <Zap size={22} />
                                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0 py-1">
                                            <div className="flex items-center justify-between mb-2">
                                                <p className="text-[10px] font-black text-foreground/50 uppercase tracking-[0.2em] flex items-center gap-1.5">
                                                    <Clock size={12} className="opacity-50" />{' '}
                                                    {formatRelativeTime(calc.createdDate)}
                                                </p>
                                            ...
                                            <div className="flex items-center gap-2 mt-1.5">
                                                <span className="px-2 py-0.5 bg-foreground/5 rounded-md text-[9px] font-black uppercase text-foreground/40 tracking-widest border border-border-theme">
                                                    {OBJECT_TYPES.find((t) => t.value === calc.type)
                                                        ?.label ||
                                                        calc.type ||
                                                        'Объект'}
                                                </span>
                                                <span className="text-[10px] font-bold text-foreground/20 italic">
                                                    #
                                                    {String(calc.project_number || '').padStart(
                                                        3,
                                                        '0'
                                                    ) || idx + 1}
                                                </span>
