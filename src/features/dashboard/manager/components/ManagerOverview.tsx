...
export const ManagerOverview = React.memo<ManagerOverviewProps>(({ calculations, onNavigate }) => {
    const stats = useMemo(() => {
        ...
        const implementation = calculations.filter((c) =>
        ...
        return {
            ...
            conversion:
                calculations.length > 0
                    ? Math.round(
                          ((invoiced.length + implementation.length) / calculations.length) * 100
                      )
                    : 0,
            avgDealSize:
                calculations.length > 0
                    ? Math.round(
                          calculations.reduce((sum, c) => sum + (c.totalCost || 0), 0) /
                              calculations.length
                      )
                    : 0,
        };
                ...
                <div className="flex items-center gap-4 bg-card border border-border-theme p-4 rounded-[2rem] shadow-xl">
                    <div className="flex -space-x-3">
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div
                                key={i}
                                className="w-10 h-10 rounded-full border-2 border-card bg-foreground/5 flex items-center justify-center overflow-hidden"
                            >
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-foreground/40 pr-4">
                        Клиентов онлайн
                    </p>
                </div>
            </div>
                ...
                {[
                    ...
                    >
                        <div className="flex justify-between items-start mb-8">
                            <div
                                className={`p-4 rounded-2xl bg-foreground/5 ${stat.color} group-hover:scale-110 transition-transform`}
                            >
                                <stat.icon size={24} />
                            </div>
                            <div
                                className={`flex items-center gap-1 text-[10px] font-black px-3 py-1.5 rounded-full ${stat.isUp ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}
                            >
                                {stat.isUp ? (
                        ...
                        <div>
                            <p className="text-4xl font-black tracking-tighter mb-1 select-none">
                                {stat.value}
                            </p>
                            <p className="text-[10px] font-black text-foreground/30 uppercase tracking-[0.2em]">
                                {stat.label}
                            </p>
                            <p className="text-[9px] font-bold text-foreground/10 uppercase mt-1 italic">
                                {stat.sub}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="glass-card p-10 overflow-hidden relative">
                        <div className="relative z-10">
                            <h3 className="text-xl font-black uppercase tracking-tight mb-8 flex items-center gap-3">
                                <PieChart className="text-primary" size={20} /> Распределение по
                                статусам
                            </h3>
                                ...
                                {[
                                    {
                                        ...
                                        percent:
                                            Math.round(
                                                (stats.pendingCount / stats.activeProjects) * 100
                                            ) || 0,
                                    },
                                    {
                                        label: 'Анализ',
                                        count: stats.changesCount,
                                        color: 'bg-primary',
                                        percent:
                                            Math.round(
                                                (stats.changesCount / stats.activeProjects) * 100
                                            ) || 0,
                                    },
                                    {
                                        label: 'Одобрено',
                                        ...
                                        percent:
                                            Math.round(
                                                (stats.approvedCount / stats.activeProjects) * 100
                                            ) || 0,
                                    },
                                    {
                                        label: 'Реализация',
                                        count: stats.implementationCount,
                                        color: 'bg-indigo-500',
                                        percent:
                                            Math.round(
                                                (stats.implementationCount / stats.activeProjects) *
                                                    100
                                            ) || 0,
                                    },
                                ].map((item, i) => (
                                    <div key={i} className="space-y-4">
                                        <div className="flex items-end justify-between">
                                            ...
                                            <p className="text-[10px] font-black text-foreground/30">
                                                {item.percent}%
                                            </p>
                                        </div>
                        ...
                        >
                            <div className="relative z-10 space-y-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-background/40 mt-2">
                                        Нормативы и цены
                                    </p>
                                </div>
                        ...
                        {recentItems.length > 0 ? (
                            recentItems.map((calc) => (
                                <div key={calc.id} className="group block space-y-3 relative pl-6">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <p className="text-[10px] font-bold text-foreground/20 italic">
                                            {calc.type}
                                        </p>
