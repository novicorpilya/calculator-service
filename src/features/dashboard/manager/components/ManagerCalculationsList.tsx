...
const CalculationCard = React.memo<{
    ...
    >
        <div className="flex justify-between items-start mb-4">
            <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center transition-all duration-500 ${vm.managerId ? 'bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white' : 'bg-orange-500/10 text-orange-500 group-hover:bg-orange-500 group-hover:text-white'}`}>
                {vm.managerId ? <Briefcase size={18} className="mb-0.5" /> : <Inbox size={18} className="mb-0.5" />}
        ...
        <div className="space-y-1 mb-4">
            <h3 className="text-lg font-black uppercase tracking-tight group-hover:text-primary transition-colors leading-tight truncate">{vm.organizationName}</h3>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border-theme">
            <div className="space-y-1">
                <p className="text-[8px] font-black text-foreground/20 uppercase tracking-widest">Объект</p>
            ...
            <div className="space-y-1 text-right">
                <p className="text-[8px] font-black text-foreground/20 uppercase tracking-widest">Бюджет</p>
        ...
        <div className="mt-4 flex items-center justify-between py-2 px-3 bg-background rounded-xl border border-border-theme group-hover:border-primary/30 transition-all">
            ...
            <ChevronRight size={14} className="text-foreground/20 group-hover:text-primary group-hover:translate-x-1 transition-all" />
        </div>
    </div>
));
...
const CalculationRow = React.memo<{
    ...
    >
        <div className="flex items-center gap-5 flex-1 min-w-0">
            <div className={`w-10 h-10 rounded-lg flex flex-col items-center justify-center shrink-0 ${vm.managerId ? 'bg-primary/10 text-primary' : 'bg-orange-500/10 text-orange-500'}`}>
                {vm.managerId ? <Briefcase size={16} /> : <Inbox size={16} />}
                <span className="text-[7px] font-black opacity-60">#{String(index + 1).padStart(3, '0')}</span>
            </div>
            <div className="min-w-0 flex-1">
                <h4 className="text-base font-black uppercase tracking-tight truncate mb-0.5">{vm.organizationName}</h4>
                <div className="flex flex-wrap items-center gap-3 text-[8px] font-black text-foreground/30 uppercase tracking-widest">
                    <span className="flex items-center gap-1"><Calendar size={9} className="text-primary" /> {vm.formattedDate}</span>
                    <span className="w-1 h-1 bg-foreground/10 rounded-full" />
                    <span className="flex items-center gap-1"><MapPin size={9} className="text-primary" /> {vm.type || '—'}</span>
                    <span className="w-1 h-1 bg-foreground/10 rounded-full" />
        ...
        <div className="flex items-center gap-8 text-right shrink-0">
            ...
            <ModernStatusBadge status={vm.status} />
            <div className="w-28">
                <p className="text-base font-black tracking-tighter leading-none">{vm.totalCostDisplay}</p>
                <p className="text-[7px] font-black text-foreground/20 uppercase tracking-[0.2em] mt-0.5">Итого</p>
            </div>
...
export const ManagerCalculationsList = React.memo<ManagerCalculationsListProps>(({
    ...
    const stats = useMemo(() => ({
        total,
        budget: viewModels.reduce((s, c) => s + (c.totalCost || 0), 0)
    }), [total, viewModels]);
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div className="space-y-4">
                    <div className="space-y-1">
                        <h1 className="text-3xl lg:text-4xl font-black uppercase tracking-tighter">Воронка проектов</h1>
                ...
                <div className="flex items-center gap-6 bg-card border border-border-theme p-4 rounded-[1.5rem] shadow-xl">
                    <div className="text-center px-2">
                        <p className="text-[8px] font-black text-foreground/30 uppercase tracking-widest mb-0.5">Всего</p>
                        <p className="text-xl font-black leading-none">{stats.total}</p>
                    </div>
                    <div className="w-[1px] h-8 bg-border-theme" />
                    <div className="text-center px-2">
                        <p className="text-[8px] font-black text-foreground/30 uppercase tracking-widest mb-0.5">Бюджет</p>
                        <p className="text-xl font-black leading-none">{(stats.budget / 1000000).toFixed(1)}M</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 bg-card border border-border-theme p-3 rounded-[1.5rem] shadow-xl">
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        <ArrowUpDown size={12} /> {pagination.sortOrder === 'desc' ? 'Новые' : 'Старые'}
                    </button>
            ...
            {isLoading ? (
                ...
                <div className="py-24 text-center bg-card/50 border-2 border-dashed border-border-theme rounded-[2rem]">
                    <div className="w-16 h-16 bg-primary/5 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4 text-primary/20">
                        {pagination.tab === 'my' ? <Briefcase size={32} /> : pagination.tab === 'unassigned' ? <Inbox size={32} /> : <Search size={32} />}
                    </div>
                    <h3 className="text-base font-black uppercase tracking-widest text-foreground/40">Ничего не найдено</h3>
                    <p className="text-[9px] font-bold text-foreground/20 uppercase tracking-[0.3em] mt-1">Измените параметры поиска</p>
                </div>
            ) : (
                <div
                    ref={parentRef}
                    ...
                    >
                        {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                            if (viewMode === 'grid') {
                                ...
                                const rowItems = viewModels.slice(startIndex, startIndex + columns);
