import {
    FileText, Send, AlertCircle, Download,
    RefreshCcw, Receipt, CreditCard, Truck,
    CheckCircle2, Lock, UserCheck, Package, type LucideIcon
} from 'lucide-react';
...
export const STATUS_UI_CONFIG: Record<CalculationStatus | 'exported', { label: string; bg: string; text: string; dot: string; icon: LucideIcon }> = {
    ...
    expert: { label: 'На экспертизе', bg: 'bg-blue-100', text: 'text-blue-600', dot: 'bg-blue-500', icon: UserCheck },
    ...
    suppliers: { label: 'У поставщиков', bg: 'bg-orange-100', text: 'text-orange-600', dot: 'bg-orange-500', icon: Package },
    ...
    ready: { label: 'Готово', bg: 'bg-green-100', text: 'text-green-600', dot: 'bg-green-500', icon: CheckCircle2 },
