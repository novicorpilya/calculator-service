...
export const RecipientListItem: React.FC<RecipientListItemProps> = React.memo(({
    ...
    return (
        <button
            onClick={onClick}
            className={`w-full p-4 flex items-center gap-3 transition-all hover:bg-primary/5 ${isSelected ? 'bg-primary/10 border-r-2 border-primary' : ''
                }`}
        >
            ...
            <div className="relative shrink-0">
                ...
                <div
                    className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-background ${isOnline ? 'bg-emerald-500' : 'bg-gray-400'
                        }`}
                />
            </div>
