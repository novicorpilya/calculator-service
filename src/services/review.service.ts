...
export interface IReviewService {
    submitReview(calculationId: string, userId: string, rating: number, comment?: string): Promise<ActionResult<ProjectReview>>;
...
export class ReviewService {
    ...
    async submitReview(
        ...
        try {
            ...
            return { 
                success: false, 
                error: { message: error instanceof Error ? error.message : 'Ошибка при сохранении отзыва' } 
            };
        }
    }

    async getReviewByCalculation(calculationId: string): Promise<ActionResult<ProjectReview | null>> {
        try {
            const { data, error } = await this.supabase
                .from('calculation_reviews')
                .select('*')
