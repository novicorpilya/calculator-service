...
const SupplierLogo: React.FC<{ logo?: string | null; name: string; className?: string }> = ({
    logo,
    name,
    className = 'w-16 h-16',
}) => {
    ...
    if (logo && !error) {
        return (
            <div
                className={`${className} rounded-2xl overflow-hidden border border-border-theme bg-white p-2 shadow-sm`}
            >
                <img
                    src={logo}
                    alt={name}
    ...
    return (
        <div
            className={`${className} rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-black text-2xl border border-primary/20 shadow-sm transition-all`}
        >
            ...
            {loading ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {[...Array(4)].map((_, i) => (
                        <div
                            key={i}
                            className="bg-card h-64 rounded-[2.5rem] animate-pulse border border-border-theme"
                        />
                    ))}
                </div>
            ) : suppliers.length === 0 ? (
                <div className="bg-card/50 border-2 border-dashed border-border-theme rounded-[2.5rem] py-32 flex flex-col items-center justify-center gap-6">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-muted-foreground">
                            Пока не добавлено ни одного поставщика
                        </p>
                    </div>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {suppliers.map((supplier) => (
                        <div
                            key={supplier.id}
                            className="group relative bg-card border border-border-theme hover:border-primary/20 rounded-[2.5rem] p-8 transition-all hover:shadow-2xl hover:shadow-primary/5"
                        >
                            <div className="flex gap-6">
                                <SupplierLogo logo={supplier.logo} name={supplier.name} />
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-1">
                                        <h3 className="font-black text-xl leading-none group-hover:text-primary transition-colors">
                                            {supplier.name}
                                        </h3>
                                        {supplier.rating && (
                                    ...
                                    <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                                        {supplier.description || 'Общий поставщик'}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-y-3 mt-6">
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                            ...
                                            <span className="truncate">
                                                {supplier.contacts?.website || 'No website'}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground overflow-hidden">
                                            ...
                                            <span className="truncate">
                                                {supplier.contacts?.email || 'No email'}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground overflow-hidden">
                                            <MapPin size={14} className="text-primary/50" />
                                            <span className="truncate">
                                                {supplier.contacts?.address || 'No address'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
