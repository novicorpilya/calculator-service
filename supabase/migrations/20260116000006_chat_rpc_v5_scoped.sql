-- Migration: get_chat_recipients_v5 (Scoped Visibility)
-- Logic update: Managers only see Clients they have active calculations with.
-- Clients only see Managers they have active calculations with.
-- Admins still see everyone.
-- Existing message history is always preserved.

DROP FUNCTION IF EXISTS public.get_chat_recipients_v5(UUID);
...
GRANT EXECUTE ON FUNCTION public.get_chat_recipients_v5(UUID) TO authenticated;

COMMENT ON FUNCTION public.get_chat_recipients_v5 IS 'Get chat recipients v5. Scoped by project (calculation) assignment. Managers see only their clients.';
