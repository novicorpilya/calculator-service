-- ============================================================
-- Migration: Add Archive and Restore actions to state machine
-- Date: 2026-01-18
-- ============================================================

-- 1. Add template for CLOSED status (Archive)
INSERT INTO public.sys_message_templates (trigger_status, body_markdown, action_config)
VALUES 
(
    'CLOSED', 
    E'Проект №{{projectNo}} («{{organizationName}}») перемещён в архив 📁\n\n━━━━━━━━━━━━━━━━━━━\n\nРабота по проекту полностью завершена и он переведен в архивное хранилище.',
    '{"type": "info_card", "icon": "archive"}'
)
ON CONFLICT (trigger_status) DO UPDATE SET
    body_markdown = EXCLUDED.body_markdown,
    action_config = EXCLUDED.action_config;

-- 2. Update the perform_calculation_action function to handle archive and restore
...
BEGIN
    ...
    IF v_new_status IN ('expert', 'paid', 'payment_review', 'payment_rejected', 'invoice', 'processing', 'sent_to_warehouse', 'ready', 'shipping', 'completed', 'closed') THEN
