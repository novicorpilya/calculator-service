-- Обновленная функция с поддержкой оплаты и кнопками для менеджера
...
BEGIN
    ...
    -- Здесь мы отправляем системное сообщение при ключевых сменах статуса
    IF v_new_status IN ('expert', 'paid', 'payment_review', 'invoice') THEN
        ...
        -- Ищем шаблон (используем имя действия или статуса как ключ)
        -- Попробуем найти или по статусу, или по действию
        ...
        IF v_template_body IS NOT NULL THEN
            ...
            -- Определяем message_type из конфига, если есть
